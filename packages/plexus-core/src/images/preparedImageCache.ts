...
import type { PharoLauncherMcpToolClient } from "../launcher/pharoLauncherMcpClient.js";
import type {
  ProjectConfig,
  ProjectImageConfig,
...
import { projectConfigId } from "../config/projectConfig.js";
import {
...
export function preparedImageLoadStatusPath(options: {
  projectRoot: string;
  projectId: string;
  cacheId: string;
  stateRoot?: string;
  ...
  return joinPathLike(
    projectPreparedImagesDirectoryPath(options),
    `prepared-image-load-${options.cacheId}.properties`,
  );
}
...
export function generatePreparedImageCacheScript(
  ...
  const loadBlock = options.loadStatusPath
    ? `loadStatusFile := ${smalltalkPath(options.loadStatusPath)} asFileReference.
loadStatusFile exists ifTrue: [ loadStatusFile delete ].
loadSource := 'pending'.
loadStatusWriter := [ :status :message |
  loadStatusFile parent ensureCreateDirectory.
  loadStatusFile writeStreamDo: [ :stream |
    stream nextPutAll: 'status='; nextPutAll: status; cr.
    stream nextPutAll: 'cacheId='; nextPutAll: ${smalltalkString(options.preparedImage.id)}; cr.
    stream nextPutAll: 'source='; nextPutAll: (loadSource ifNil: [ 'unknown' ]); cr.
    stream nextPutAll: 'loadScript='; nextPutAll: ${smalltalkPath(loadScriptPath)}; cr.
    stream nextPutAll: 'repository='; nextPutAll: ${smalltalkString(repositoryLabel)}; cr.
    stream nextPutAll: 'baseline='; nextPutAll: ${smalltalkString(repository.baseline)}; cr.
    message ifNotNil: [
...
[
  ...
  loadScript exists
    ifTrue: [
      loadSource := 'loadScript'.
      loadScript fileIn ]
    ifFalse: [
      loadSource := 'metacello'.
      Metacello new
  ...
  loadStatusWriter value: 'loaded' value: nil
] on: Error do: [ :error |
...
| loadScript loadStatusFile loadStatusWriter loadSource |
...
export function writePreparedImageCacheScript(
...
): { filePath: string; source: string; loadStatusPath: string } {
  const preparedImage = findPreparedImageConfig(options.config, options.cacheId);
  const stateRoot = projectStateRootForConfig(options.config, options.stateRoot);
  ...
  const loadStatusPath = preparedImageLoadStatusPath({
    projectRoot: options.projectRoot,
    projectId: projectConfigId(options.config),
    cacheId: preparedImage.id,
    stateRoot,
  ...
  return { filePath, source, loadStatusPath };
}
