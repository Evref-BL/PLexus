...
} from "../workspace/projectStartupScript.js";
import {
  projectConfigId,
...
export function homeImageCachePreparationStatusPath(
  cacheRoot: string,
  key: string,
): string {
  return joinPathLike(
    homeImageCacheEntryDirectory(cacheRoot, key),
    "preparation-status.properties",
  );
}
...
| loadScript preparationStatusFile preparationStatusWriter preparationSource |

Smalltalk globals at: #PLexusHomeImageCacheKey put: ${quoted(options.cacheId)}.

preparationStatusFile := ${pathString(options.statusPath)} asFileReference.
preparationStatusFile exists ifTrue: [ preparationStatusFile delete ].
preparationSource := 'pending'.
preparationStatusWriter := [ :status :message |
  preparationStatusFile parent ensureCreateDirectory.
  preparationStatusFile writeStreamDo: [ :stream |
    stream nextPutAll: 'status='; nextPutAll: status; cr.
    stream nextPutAll: 'cacheKey='; nextPutAll: ${quoted(options.cacheId)}; cr.
    stream
      nextPutAll: 'source=';
      nextPutAll: (preparationSource ifNil: [ 'unknown' ]);
      cr.
    stream nextPutAll: 'loadScript='; nextPutAll: ${pathString(loadScriptPath)}; cr.
    stream nextPutAll: 'repository='; nextPutAll: ${quoted(repositoryLabel)}; cr.
    stream nextPutAll: 'baseline='; nextPutAll: ${quoted(repository.baseline)}; cr.
    message ifNotNil: [
...
[
  ...
  loadScript exists
    ifTrue: [
      preparationSource := 'loadScript'.
      loadScript fileIn ]
    ifFalse: [
      preparationSource := 'metacello'.
      Metacello new
  ...
  preparationStatusWriter value: 'loaded' value: nil
] on: Error do: [ :error |
...
export function buildHomeImageCachePlan(
  ...
  const preparationStatusPath = homeImageCachePreparationStatusPath(cacheRoot, key);
...
export function writeHomeImageCachePreparationScript(plan: HomeImageCachePlan): {
  ...
  const source = preparationScriptSource({
    ...
    source: plan.source,
    mcp: plan.mcp,
    statusPath: plan.expectedManifest.paths.preparationStatusPath,
  });
...
function preparationStatusProperties(
  filePath: string,
...
function preparationFailureDiagnostic(plan: HomeImageCachePlan): string | undefined {
  const properties = preparationStatusProperties(
    plan.expectedManifest.paths.preparationStatusPath,
  );
  ...
  const message =
    properties.message ??
    "Pharo MCP preparation failed without a reported message.";
  return `Home image cache Pharo MCP preparation failed: ${message}`;
}
...
async function prepareHomeImageCacheEntry(options: {
  ...
  try {
    if (options.plan.prepareCacheImage) {
      fs.rmSync(options.plan.expectedManifest.paths.preparationStatusPath, {
        force: true,
      });
    ...
    writeHomeImageCacheManifest(
      finalizedManifest({
        ...
        diagnostics: [
          preparationFailureDiagnostic(options.plan) ??
